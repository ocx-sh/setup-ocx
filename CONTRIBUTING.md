...
| `src/version.ts`         | Version resolution (`"latest"` via GitHub API, with retry) + `compareVersions()` semver floor |
...
| `src/managed-config.ts`  | `[managed]` config tier adoption via `ocx config setup`                                       |
| `src/http-retry.ts`      | Generic `withRetry()` helper (exp. backoff + Retry-After)                                     |
| `src/win-title-guard.ts` | Side-effect import — preempts the libuv `process_title` abort on Windows                      |
