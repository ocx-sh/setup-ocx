...
- Generate `licenses.txt` with `ncc --license` to track third-party licenses in the bundle.
...
- Bundle everything with ncc so no `npm install` is needed at runtime.
- Use `@actions/http-client` instead of heavier alternatives (axios, node-fetch).
- `@actions/exec` is listed as a dependency but unused — remove unused dependencies.
...
### Bundling
- Use `@vercel/ncc` to compile TypeScript and all dependencies into a single `dist/setup/index.js`.
- Never commit `node_modules/`.
- Add `--license` flag to ncc to generate `licenses.txt` alongside the bundle.
